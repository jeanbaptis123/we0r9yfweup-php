...
function keycheck(serialkey) {
    fetch("https://api.proxyscrape.com/?request=keystatus&serialkey=" + serialkey).then(response => response.text()).then(function(response) {
        ...
        if (response.includes("Invalid serial key")) {
            ...
            nonepremium = document.getElementById("nonepremium");
            nonepremium.style.display = "none";
            toggle('premium', 'block');
            toggle('fakesliders', 'none');
            usedserialkey = serialkey;
            httpbuilder();
            fetch("https://api.proxyscrape.com/?request=lastupdated&proxytype=http" + (ispremium ? "&serialkey=" + usedserialkey : "")).then(response => response.text()).then(data => $("#httplastupdated").html(data))
            socks4builder();
            fetch("https://api.proxyscrape.com/?request=lastupdated&proxytype=socks4" + (ispremium ? "&serialkey=" + usedserialkey : "")).then(response => response.text()).then(data => $("#socks4lastupdated").html(data))
            socks5builder();
            fetch("https://api.proxyscrape.com/?request=lastupdated&proxytype=socks5" + (ispremium ? "&serialkey=" + usedserialkey : "")).then(response => response.text()).then(data => $("#socks5lastupdated").html(data))
        } else {
            msgbox.style.display = "block";
            login.style.display = "none";
...
function httpbuilder() {
    ...
    if (ispremium) {
        httpbuild.push("&uptime=" + httpuptime);
        httpbuild.push("&status=" + httpstatus);
        httpbuild.push("&averagetimeout=" + httpaveragetimeout);
        httpbuild.push("&age=" + httpage);
        httpbuild.push("&score=" + httpscore);
        httpbuild.push("&port=" + httpport);
        httpbuild.push("&organization=" + httporganization);
        httpbuild.push("&serialkey=" + usedserialkey);
    }
    const waitforrequest = setInterval(() => {
        if (httpisrequesting == false) {
            if (httptotalrequests == currentrequest) {
                httpisrequesting = true
                fetch("https://api.proxyscrape.com/?request=amountproxies&proxytype=http" + httpbuild.join("")).then(response => response.json()).then(data => $("#httpproxyamount").html(data))
                httpisrequesting = false
                document.querySelector("#downloadhttp").href = "https://api.proxyscrape.com/?request=getproxies&proxytype=http" + httpbuild.join("");
                document.querySelector("#sharehttp").href = "https://api.proxyscrape.com/?request=share&proxytype=http" + httpbuild.join("");
                document.querySelector("#downloadiconhttp").href = "https://api.proxyscrape.com/?request=getproxies&proxytype=http" + httpbuild.join("");
...
function socks4builder() {
    ...
    if (ispremium) {
        socks4build.push("&uptime=" + socks4uptime);
        socks4build.push("&status=" + socks4status);
        socks4build.push("&averagetimeout=" + socks4averagetimeout);
        socks4build.push("&age=" + socks4age);
        socks4build.push("&score=" + socks4score);
        socks4build.push("&port=" + socks4port);
        socks4build.push("&organization=" + socks4organization);
        socks4build.push("&serialkey=" + usedserialkey);
    }
    const waitforrequest = setInterval(() => {
        if (socks4isrequesting == false) {
            if (socks4totalrequests == currentrequest) {
                socks4isrequesting = true
                fetch("https://api.proxyscrape.com/?request=amountproxies&proxytype=socks4" + socks4build.join("")).then(response => response.json()).then(data => $("#socks4proxyamount").html(data))
                socks4isrequesting = false
                document.querySelector("#downloadsocks4").href = "https://api.proxyscrape.com/?request=getproxies&proxytype=socks4" + socks4build.join("");
                document.querySelector("#sharesocks4").href = "https://api.proxyscrape.com/?request=share&proxytype=socks4" + socks4build.join("");
                document.querySelector("#downloadiconsocks4").href = "https://api.proxyscrape.com/?request=getproxies&proxytype=socks4" + socks4build.join("");
                clearInterval(waitforrequest);
            } else {
                console.log("request dropped");
                clearInterval(waitforrequest);
...
function socks5builder() {
    ...
    if (ispremium) {
        socks5build.push("&uptime=" + socks5uptime);
        socks5build.push("&status=" + socks5status);
        socks5build.push("&averagetimeout=" + socks5averagetimeout);
        socks5build.push("&age=" + socks5age);
        socks5build.push("&score=" + socks5score);
        socks5build.push("&port=" + socks5port);
        socks5build.push("&organization=" + socks5organization);
        socks5build.push("&serialkey=" + usedserialkey);
    }
    const waitforrequest = setInterval(() => {
        if (socks5isrequesting == false) {
            if (socks5totalrequests == currentrequest) {
                socks5isrequesting = true
                fetch("https://api.proxyscrape.com/?request=amountproxies&proxytype=socks5" + socks5build.join("")).then(response => response.json()).then(data => $("#socks5proxyamount").html(data))
                socks5isrequesting = false
                document.querySelector("#downloadsocks5").href = "https://api.proxyscrape.com/?request=getproxies&proxytype=socks5" + socks5build.join("");
                document.querySelector("#sharesocks5").href = "https://api.proxyscrape.com/?request=share&proxytype=socks5" + socks5build.join("");
                document.querySelector("#downloadiconsocks5").href = "https://api.proxyscrape.com/?request=getproxies&proxytype=socks5" + socks5build.join("");
                clearInterval(waitforrequest);
            } else {
                console.log("request dropped");
                clearInterval(waitforrequest);
...
$(this).one('scroll', function(){
  fetch("https://api.proxyscrape.com/?request=getlist").then(response => response.text()).then(function(response) {
    var obj = JSON.parse(response);

    for (i = 0; i < httporganizationlist.length; i++) {
        httporganizationlist[i].innerHTML = "<li data-value='all' class='option'>all</li>";
        Object.keys(obj['http']['organization']).forEach(function(entry) {
    ...
    for (i = 0; i < httpcountrylist.length; i++) {
        ...
        Object.keys(obj['http']['country']).forEach(function(entry) {
    ...
    for (i = 0; i < httpportlist.length; i++) {
        ...
        Object.keys(obj['http']['port']).forEach(function(entry) {
    ...
    for (i = 0; i < socks4organizationlist.length; i++) {
        ...
        Object.keys(obj['socks4']['organization']).forEach(function(entry) {
    ...
    for (i = 0; i < socks4countrylist.length; i++) {
        ...
        Object.keys(obj['socks4']['country']).forEach(function(entry) {
    ...
    for (i = 0; i < socks4portlist.length; i++) {
        ...
        Object.keys(obj['socks4']['port']).forEach(function(entry) {
    ...
    for (i = 0; i < socks5organizationlist.length; i++) {
        ...
        Object.keys(obj['socks5']['organization']).forEach(function(entry) {
    ...
    for (i = 0; i < socks5countrylist.length; i++) {
        ...
        Object.keys(obj['socks5']['country']).forEach(function(entry) {
    ...
    for (i = 0; i < socks5portlist.length; i++) {
        ...
        Object.keys(obj['socks5']['port']).forEach(function(entry) {
